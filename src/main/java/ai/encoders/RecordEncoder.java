...
import ai.record.RecordColumn.Type;
	...
	public RecordEncoder<V> initiate(Class<?> val, int bits, boolean hours, boolean day_of_month, boolean month_of_year, boolean week_of_year) {
		...
		for(Field field : fields) {
			...
			if(type.contains("double") || type.contains("float") || type.contains("int")) {							
				...
				encode_maps[count] = new TimeEncoder(field.getName(), "yyyy-MM-dd HH:mm:ss");
				if(hours) {
					((TimeEncoder) encode_maps[count]).addTimeEncoder(TimeEncoder.Time_Encoder.HOUR_OF_DAY);
				}
				if(day_of_month) {
					((TimeEncoder) encode_maps[count]).addTimeEncoder(TimeEncoder.Time_Encoder.DAY_OF_MONTH);
				}
				if(month_of_year) {
					((TimeEncoder) encode_maps[count]).addTimeEncoder(TimeEncoder.Time_Encoder.MONTH_OF_YEAR);
				}
				if(week_of_year) {
					((TimeEncoder) encode_maps[count]).addTimeEncoder(TimeEncoder.Time_Encoder.WEEK_OF_YEAR);
				}
		...
		if(record.getType() != null) {
			...
			for(int i = 0; i < record.getType().length; i++) {
				...
				if(field_types[i] == Type.TIME) {
					...
					if(hours) {
						t_encoder.addTimeEncoder(TimeEncoder.Time_Encoder.HOUR_OF_DAY);
					}
					if(day_of_month) {
						t_encoder.addTimeEncoder(TimeEncoder.Time_Encoder.DAY_OF_MONTH);
					}
					if(month_of_year) {
						t_encoder.addTimeEncoder(TimeEncoder.Time_Encoder.MONTH_OF_YEAR);
					}
					if(week_of_year) {
						t_encoder.addTimeEncoder(TimeEncoder.Time_Encoder.WEEK_OF_YEAR);
					}
